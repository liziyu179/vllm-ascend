...
def _transpose_to_workspace_kernel(
    ...
    源 block 逻辑布局为：
        [split_num, block_size, group_elems]

    workspace 中的目标布局为：
        [block_size, split_num, group_elems]
    ...
    (token, split, group)，再到源布局中的 (split, token, group) 位置读取。
    """
    ...
    # 目标线性布局：
    #   dst[token, split, group]
    #     = token * split_num * group_elems
    #       + split * group_elems
    #       + group
    dst_cell = offsets // group_elems
    group_offset = offsets - dst_cell * group_elems
    token_idx = dst_cell // split_num
    split_idx = dst_cell - token_idx * split_num

    # 源线性布局：
    #   src[split, token, group]
    #     = split * block_size * group_elems
    #       + token * group_elems
    #       + group
    src_offsets = (
        block_id * block_num_stride
        + split_idx * block_size * group_elems
        + token_idx * group_elems
        + group_offset
    )
...
def _check_cache(
    ...
    assert cache.is_contiguous(), (
        "transpose_kv_cache_by_block_triton requires contiguous KV cache "
        "tensors"
    )
    assert cache.dim() == 4, (
        "expected cache shape [num_blocks, block_size, head_num, head_dim], "
        f"got {cache.shape}"
    )

    head_num = cache.shape[2]
    head_dim = cache.shape[3]
    assert head_num % split_num == 0, (
        f"head_num={head_num} must be divisible by split_num={split_num}"
    )
    assert block_size == cache.shape[1], (
        f"block_size={block_size} does not match "
        f"cache.shape[1]={cache.shape[1]}"
    )
    assert block_ids.device == cache.device

    total_elems = block_size * head_num * head_dim
    group_elems = total_elems // (block_size * split_num)
    return group_elems, total_elems
...
def _run_for_cache(
    ...
    group_elems, total_elems = _check_cache(cache, block_ids, block_size, split_num)
