...
class SharedProxyScheduler:
    ...
    def __init__(self, prefiller_instances, decoder_instances):
        ...
        self.aborted_prefiller_requests: dict[str, set[str]] = {}
        self._ordinal = 0
    ...
    def _add_server_no_lock(self, instance_type: str, host: str, port: int) -> bool:
        ...
        if instance_type == InstanceType.PREFILL:
            self.aborted_prefiller_requests.setdefault(key, set())
        return True
    ...
    def abort_prefiller_request(self, key: str, request_id: str) -> None:
        with self._lock:
            self.aborted_prefiller_requests.setdefault(key, set()).add(request_id)

    def acquire_aborted_prefiller_requests(self, key: str) -> list[str]:
        with self._lock:
            aborted = list(self.aborted_prefiller_requests.get(key, set()))
            self.aborted_prefiller_requests[key] = set()
            return aborted
    ...
    def _remove_instances(self, instance_type: str, instances: list[tuple[str, int]]) -> bool:
        ...
        with self._lock:
            ...
            for key in keys:
                ...
                self.waiting_nodes.pop(key, None)
                if instance_type == InstanceType.PREFILL:
                    self.aborted_prefiller_requests.pop(key, None)
                    self.tainted_prefillers.discard(key)
                else:
                    self.tainted_decoders.discard(key)
            if removed:
...
async def send_request_to_service(
    ...
    aborted_requests = runtime.scheduler.acquire_aborted_prefiller_requests(prefiller_key)
